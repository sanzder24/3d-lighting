 ...
 *
 */
class LitBox_Beta extends LitRenderable3D
{
    /**
     ...
     render()
    {
        ...
        for (let i=0;i<this.indices.length;i++)
            gl.drawElements(gl.TRIANGLE_FAN, 4, gl.UNSIGNED_SHORT, i*4*2);
    }
}
...
class Sphere extends LitRenderable3D {
    ...
    constructor(shader, translation)
    {
        super(shader);
         ...
         *  Initialize JS arrays with vertices attributes
         */
        // create sphere vertices (relatively easy if you draw a picture first)
        this.vertices = []
        this.indices  = []
        var info = generate_sphere(20, this.vertices, this.indices)
        this.vertices = info[0];
        this.indices = info[1];
        this.factor = 0
        this.factorTop = 0;
    ...
    render()
    {
        ...
        gl.uniform3f(this.shader.u_LightColor, 0.8, 0.8, 0.8);
        gl.uniform3fv(this.shader.u_LightPosition_ecs,[0.0,0.0,-0.1]);
        gl.uniform3f(this.shader.u_AmbientLight, 1.0, 1.0, 1.0);
